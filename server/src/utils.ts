  ...
  const allWordsForDraw = shuffleArray(
    gameInformationSketchWars.categories.reduce(
      (result: Array<Word>, currentCategory: any) => [
        ...result,
        ...currentCategory.category.categoryWords.map(
          (categoryWord: any) => categoryWord.word
        ),
      ],
      []
    )
...
export const handleStartRound = ({
  ...
  if (serverState[eventId]) {
          ...
          rounds: team.rounds.map((round) => {
            if (round.id === roundId && round.word) {
              ...
              if (hints.length < lettersWithoutSpaceIndexes.length - 3) {
                const availableIndexes = lettersWithoutSpaceIndexes.filter(
                  (index) => !round.hints.includes(index)
                );
    ...
    console.log("RUN ROUND RESULTS PROCESSING: ", roundId);
    ...
    console.log("START ROUND", roundData);

    redisClient.publish(
      `messages:${eventId}`,
      JSON.stringify({
        event: "update-current-round",
        data: roundData,
      })
    );
    ...
    if (currentTeam && currRound) {
      try {
        ...
        redisClient.publish(
          ...
          JSON.stringify({
            event: "show-result",
            data: { roundResults: updatedPlayers },
          })
        );
        ...
        setTimeout(async () => {
          ...
          if (isLastRound) {
            ...
            console.log("RUN AUTO PICK FOR NEXT ROUND: ", nextRound.id);
              ...
              () => {
                nextRound &&
                  handleStartRound({
                    ...
                    updates: {
                      word:
                        nextRound.wordsForDraw[
                          (serverState[eventId].teams[0].rounds.length -
                            nextRound.index) %
                            3
                        ] || nextRound.wordsForDraw[0],
                      startTime: new Date().getTime(),
                      status: RoundStatuses.ONGOING,
                    },
                  });
