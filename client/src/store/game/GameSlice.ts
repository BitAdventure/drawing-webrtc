...
import { EventInfoType, LineType, PlayerResultType, RoundResults, RoundType } from "@/constants/types";

type GameInitialStateType = {
...
export const initialState: GameInitialStateType = {
  ...
  // for case when user set system time manually
  timeDifference: 0,
...
const GameSlice = createSlice({
  ...
  initialState,
  reducers: {
    setTimeDifference: (state, { payload }: { payload: number }) => {
      state.timeDifference = payload;
      state.timeDifferenceLoading = false;
    },
    updateStoreEventInfo: (state, { payload }: { payload: { eventInfo: any; withRoundsUpdate?: boolean } }) => {
      ...
      if (payload.withRoundsUpdate) {
        ...
        const currRound = rounds.find((round) => round.status !== RoundStatuses.COMPLETED) || rounds[rounds.length - 1];
        ...
        modifiedEventInfo.gameInformation.categories.includes(state.allWordsCategoryId)
      ) {
        modifiedEventInfo.gameInformation.categories = state.wordCategories.map((category) =>
          category.value.toString()
        );
      }

      state.eventInfo = modifiedEventInfo;
      if (state.eventInfoSubscriptionLoading) state.eventInfoSubscriptionLoading = false;
    },
    updatePartialCurrentRound: (state, { payload }: { payload: Partial<RoundType> }) => {
      if (payload.correctAnswers) {
        state.isAllPlayersGuessTheWord =
          !!payload.correctAnswers.length &&
          payload.correctAnswers.length === (state.eventInfo?.team.players.length || 0) - 1;
      }
    ...
    updateCurrentRound: (state, { payload }: { payload: RoundType }) => {
      state.currentRound = {
        ...payload,
        startTime: payload.startTime && payload.startTime + state.timeDifference,
      };
    },
    updateRoundResults: (state, { payload: { roundResults } }: { payload: { roundResults: RoundResults } }) => {
    ...
    updateLines: (state, { payload }: { payload: Array<LineType> }) => {
      if (state.currentRound) {
        state.currentRound.lines = payload;
      }
    },
